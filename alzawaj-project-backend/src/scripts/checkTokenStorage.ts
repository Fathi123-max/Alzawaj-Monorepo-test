...
async function checkTokenStorage() {
  try {
    ...
    if (user.emailVerificationToken) {
      ...
      const hashOfOriginal = crypto.createHash('sha256').update(originalValue).digest('hex');
      
      console.log('Original token length (6gfyp9uqgxt6mw7ti6c3qo):', originalValue.length);
      console.log('Hash of original would be length:', hashOfOriginal.length);
      console.log('Does token look like a hash?:', user.emailVerificationToken.length === 64);
      
      // Check if the stored token matches the hash of the original
      console.log('Stored token matches hash of original?', user.emailVerificationToken === hashOfOriginal);
