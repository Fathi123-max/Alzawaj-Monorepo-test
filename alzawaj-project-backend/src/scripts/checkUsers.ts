...
async function checkUsers() {
  try {
    ...
    const randomUsers = await User.find({}).limit(5).select('email isEmailVerified _id');
    console.log('\nSome sample users:');
    randomUsers.forEach((user, index) => {
      console.log(`${index + 1}. ID: ${user._id}, Email: ${user.email}, Verified: ${user.isEmailVerified}`);
    });
