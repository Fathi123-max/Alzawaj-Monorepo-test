...
import { StorageProtection } from "@/lib/utils/storage-protection";
...
function DefaultErrorFallback({
  ...
  // Function to preserve localStorage during reset
  const handleResetWithStorage = () => {
    try {
      // Enable storage protection
      StorageProtection.enable();

      // Reset the error
      resetError();

      // Restore any cleared data and disable protection after a brief delay
      setTimeout(() => {
        StorageProtection.restore();
        StorageProtection.disable();
      }, 500);
    } catch (err) {
      console.error("Error preserving localStorage during reset:", err);
      // Ensure protection is disabled even if there's an error
      StorageProtection.disable();
      // Fallback to normal reset if localStorage preservation fails
      resetError();
    }
  };
  return (
    <div className="min-h-screen flex items-center justify-center bg-background">
      <div className="max-w-md mx-auto text-center p-6">
        ...
        <div className="space-y-3">
          <button
            onClick={handleResetWithStorage}
