...
export const termsUtils = {
   ...
   */
  getTermsAcceptedDate(): Date | null {
    if (typeof window === "undefined") return null;

    try {
      const dateString = localStorage.getItem("termsAcceptedDate");
      return dateString ? new Date(dateString) : null;
    } catch (error) {
      console.warn(
        "Failed to read terms acceptance date from localStorage:",
        error,
      );
      return null;
    }
  ...
  clearTermsAcceptance(): void {
    ...
    try {
      ...
      console.warn(
        "Failed to clear terms acceptance from localStorage:",
        error,
      );
    }
  },
