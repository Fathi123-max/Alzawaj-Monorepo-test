...
test.describe("Chat Authentication Tests", () => {
  test("should properly handle authentication and connect to socket", async ({
    page,
  }) => {
    ...
    const chatElements = page.locator(
      '[class*="chat"], [data-testid*="chat"], .chat-container, .message-list',
    );
  ...
  test("should have proper auth token handling in localStorage", async ({
    page,
  }) => {
