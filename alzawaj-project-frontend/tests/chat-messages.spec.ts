...
test.describe('Chat Message Loading Tests', () => {
  test('should load chat messages when entering a chat room', async ({ page }) => {
    // Navigate to the application
    await page.goto('/');
    
    // Check if user is already logged in
    const dashboardLink = page.locator('text=الرئيسية').or(page.locator('text=Dashboard'));
    if (await dashboardLink.count() > 0) {
      ...
      console.log("Login page is accessible - would need credentials to continue");
      return;
    }
    ...
    const chatRooms = page.locator('[data-testid="chat-room"], [class*="chat-room"], [class*="room"]').first();
    ...
    if (await chatRooms.count() > 0) {
      ...
      const messageElements = page.locator('[data-testid="message"], [class*="message"], .message-bubble, .chat-message');
      ...
      if (messageCount > 0) {
        ...
        const emptyState = page.locator('text=لا توجد رسائل|No messages|Messages will appear here');
        ...
        if (await emptyState.count() > 0) {
          ...
          console.log('ℹ️ Chat room loaded but no messages or empty state found');
        }
      }
    } else {
      console.log('ℹ️ No chat rooms found - user may not have any active chats');
    }
    
    // Verify the chat interface elements are present
    const messageInput = page.locator('textarea, [data-testid="message-input"], [class*="message-input"]');
    const sendButton = page.locator('button', { hasText: /إرسال|Send/i }).or(
      page.locator('[data-testid="send-button"], [class*="send-button"]')
    );
    
    // These should be available regardless of whether messages exist
    await expect(messageInput).toBeVisible().catch(() => {
      console.log('Message input not found, which may be expected depending on user permissions');
    });
    
    await expect(sendButton).toBeVisible().catch(() => {
      console.log('Send button not found, which may be expected depending on user permissions');
    });
  });

  test('should display loading state while fetching messages', async ({ page }) => {
    // Navigate to the application
    await page.goto('/');
    ...
    const loadingIndicator = page.locator('text=جاري التحميل|Loading...|...').first();
    ...
    const chatPage = page.locator('[class*="chat"], [data-testid="chat-interface"]');
    
    if (await chatPage.count() > 0) {
      console.log('✅ Chat interface structure is present');
    } else {
      console.log('ℹ️ Chat interface not found - may need to navigate to chat section first');
    }
  });
  ...
  test('should handle empty chat room gracefully', async ({ page }) => {
    ...
    const chatRoom = page.locator('[data-testid="chat-room"], [class*="chat-room"]').first();
    ...
    if (await chatRoom.count() > 0) {
      ...
      const emptyState = page.locator('text=لا توجد رسائل|No messages yet|No messages in this chat');
      const messageCount = await page.locator('[data-testid="message"], [class*="message"]').count();
      
      console.log(`Message count: ${messageCount}`);
      
      if (messageCount === 0) {
        if (await emptyState.count() > 0) {
          console.log('✅ Empty state correctly displayed when no messages exist');
        } else {
          console.log('ℹ️ No messages found, but no specific empty state text visible either');
        }
      } else {
